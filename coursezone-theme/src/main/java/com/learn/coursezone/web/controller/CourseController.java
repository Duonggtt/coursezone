...
import org.youngmonkeys.ecommerce.web.response.WebProductCategoryParentResponse;
...
public class CourseController {
    ...
    public View classesGet(
        ...
        EClassFilter filter = CourseZoneEClassFilter.builder()
            .statuses(Arrays.asList(EClassStatus.SHOW, EClassStatus.REGISTER_OPENED, EClassStatus.VIDEO_OPENED))
        ...
        PaginationModel<WebEClassResponse> pagination = this.eclassControllerService
                .getClasses(userId, filter, nextPageToken, prevPageToken, lastPage, limit, defaultCurrency.getId(), defaultCurrency.getFormat());
        List<ProductCategoryModel> categories = this.productCategoryService.getProductCategoriesByTypeAndStatuses("CATEGORY", Collections.singletonList("SHOW"));
